...
async def cmd(ctx, cmds):
    if cmds == "av":
        ...
        embed.add_field(name="Well to see the avatar's of users",
                        value="in big picture",
                        inline=False)
        embed.add_field(name='Use for this cmd:',
                        value="`?av <user>`",
                        inline=False)
        await ctx.channel.send(embed=embed)
        ...
        embed.add_field(name="Tag me to get affirmations",
                        value="Let me try to make ur day",
                        inline=False)
        await ctx.channel.send(embed=embed)

    elif cmds == "whois":
        embed = discord.Embed(title="Waaait a minute... Who are you?",
                              color=0x9BF080)
        embed.add_field(name="Wanna know about some user",
                        value="You came to the right place",
                        inline=False)
        embed.add_field(name='Use for this cmd:',
                        value="`?whois <user>`",
                        inline=False)
...
async def aid(ctx):
    embed = discord.Embed(title='Helping your brain here',
...
async def on_message(message):
    ...
    if client.user in message.mentions:
        ...
        if is_it_me(
            ...
            embed.add_field(name="Bot Name",
                            value=f"{bot_user.name}#{bot_user.discriminator}",
                            inline=False)
            ...
            embed.add_field(name="Invite Link",
                            ...
                            inline=False)
            embed.add_field(name="Server Permissions",
                            value=perm_text[:1024],
                            inline=False)
...
async def on_voice_state_update(member, before, after):
    ...
    if (before.channel is None or before.channel.id != target_vc_id) and (
        ...
        if send_channel and member.id in user_goofy_lines:
            user_lines = [
